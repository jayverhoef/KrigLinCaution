...
#' Cressie's weighted least squares function for a variogram model
#'
#' Cressie's weighted least squares function for a variogram model
#'
...
#'
#' @author Jay Ver Hoef
vgrmFitCWLS = function(theta, aCorMod, empsvgm, dist, npair, useNugget = TRUE) {
  svgmMod = exp(theta[1])*(1 - aCorMod(dist, exp(theta[2])))
  if(useNugget == TRUE) svgmMod = svgmMod + exp(theta[3])
  sum(npair*(empsvgm/svgmMod - 1)^2)
}